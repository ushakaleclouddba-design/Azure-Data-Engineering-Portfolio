...
#
# Critical resilience pattern: each server is wrapped in try/catch. If SomeServer
# fails (e.g. SSPI hiccup, network blip, dead service), we log the error,
# move on to AnotherServer, and the Excel still gets built with whatever succeeded.
...
# -----------------------------------------------------------------------------
# AI ESTATE SYNTHESIS (auto-generated narrative)
# -----------------------------------------------------------------------------
$rowIdx++   # gap
...
# =============================================================================
# Quick-reference orientation tab for anyone opening the report cold.
# Designed to be readable in 60 seconds. Companion to the full Word document
# titled "Architecture_Board_Guide.docx" which lives outside the workbook.

$how = $pkg.Workbook.Worksheets.Add('How to Read This Report')
$hr = 1

# Title banner
$how.Cells[$hr, 1].Value = 'How to Read This Report'
$how.Cells[$hr, 1, $hr, 4].Merge = $true
$how.Cells[$hr, 1].Style.Font.Bold = $true
$how.Cells[$hr, 1].Style.Font.Size = 16
$how.Cells[$hr, 1].Style.Font.Color.SetColor([System.Drawing.ColorTranslator]::FromHtml('#FFFFFF'))
$how.Cells[$hr, 1].Style.Fill.PatternType = [OfficeOpenXml.Style.ExcelFillStyle]::Solid
$how.Cells[$hr, 1].Style.Fill.BackgroundColor.SetColor([System.Drawing.ColorTranslator]::FromHtml($NavyHex))
$how.Cells[$hr, 1].Style.VerticalAlignment = [OfficeOpenXml.Style.ExcelVerticalAlignment]::Center
$how.Row($hr).Height = 32
$hr += 2

# Subtitle
$how.Cells[$hr, 1].Value = '60-second orientation. For deeper detail, see the Architecture Board Guide (Word doc).'
$how.Cells[$hr, 1, $hr, 4].Merge = $true
$how.Cells[$hr, 1].Style.Font.Italic = $true
$how.Cells[$hr, 1].Style.Font.Color.SetColor([System.Drawing.ColorTranslator]::FromHtml('#595959'))
$hr += 2

# ---- Section: What is this report? ----
$how.Cells[$hr, 1].Value = 'What is this report?'
$how.Cells[$hr, 1, $hr, 4].Merge = $true
$how.Cells[$hr, 1].Style.Font.Bold = $true
$how.Cells[$hr, 1].Style.Font.Color.SetColor([System.Drawing.ColorTranslator]::FromHtml('#FFFFFF'))
$how.Cells[$hr, 1].Style.Fill.PatternType = [OfficeOpenXml.Style.ExcelFillStyle]::Solid
$how.Cells[$hr, 1].Style.Fill.BackgroundColor.SetColor([System.Drawing.ColorTranslator]::FromHtml($NavyHex))
$how.Row($hr).Height = 22
$hr++

$intro = @(
    'A SQL Server-to-Azure migration assessment generated by an AI-driven agent.',
    'For each server, the agent decides between four migration targets (DC, Azure VM PAYG, Azure VM BYOL, Azure SQL Managed Instance), calculates the 1-year cost of each, identifies findings that block or complicate migration, and writes a per-server recommendation narrative.',
    'AI is used for: target recommendation, MI sizing, and narrative writing only. AI is NOT used for cost calculations, severity scoring, or assessment queries.',
    'Cost numbers come from the live Azure Retail Prices API (cloud) and industry reference rates (on-prem). Both are documented in the Methodology & Sources tab.'
)
foreach ($line in $intro) {
    $how.Cells[$hr, 1].Value = "* $line"
    $how.Cells[$hr, 1, $hr, 4].Merge = $true
    $how.Cells[$hr, 1].Style.WrapText = $true
    $how.Cells[$hr, 1].Style.VerticalAlignment = [OfficeOpenXml.Style.ExcelVerticalAlignment]::Top
    $estLines = [math]::Ceiling($line.Length / 110)
    $how.Row($hr).Height = [math]::Max(20, $estLines * 16)
    $hr++
}
$hr++

# ---- Section: Reading order ----
$how.Cells[$hr, 1].Value = 'Tabs in reading order'
$how.Cells[$hr, 1, $hr, 4].Merge = $true
$how.Cells[$hr, 1].Style.Font.Bold = $true
$how.Cells[$hr, 1].Style.Font.Color.SetColor([System.Drawing.ColorTranslator]::FromHtml('#FFFFFF'))
$how.Cells[$hr, 1].Style.Fill.PatternType = [OfficeOpenXml.Style.ExcelFillStyle]::Solid
...
# Header row for tabs table
$how.Cells[$hr, 1].Value = 'Tab'
$how.Cells[$hr, 2].Value = 'Read this when...'
$how.Cells[$hr, 3].Value = 'Time to read'
for ($c = 1; $c -le 3; $c++) {
    [int]$cInt = $c
    $hcell = $how.Cells.Item($hr, $cInt)
    $hcell.Style.Font.Bold = $true
    $hcell.Style.Fill.PatternType = [OfficeOpenXml.Style.ExcelFillStyle]::Solid
    $hcell.Style.Fill.BackgroundColor.SetColor([System.Drawing.ColorTranslator]::FromHtml($LightHex))
}
...
$tabRows = @(
    ,@('Executive Summary',     'Start here. Estate at a glance, AI-generated strategy paragraph, cost summary table, top blockers.', '5 min')
    ,@('Summary',               'Need a flat per-server inventory or estate totals (DB count, jobs, AGs, etc).',                          '3 min')
    ,@('Critical Findings',     'Want every High-severity finding across the estate in one sortable list.',                              '5 min')
    ,@('Remediation Plan',      'Need to scope the work to address the High findings before migration.',                                  '5 min')
    ,@('Methodology & Sources', 'A reviewer asks "where did this number come from" or wants to defend the report.',                       '5 min')
    ,@('How to Read This Report', 'You are reading it now.',                                                                              '1 min')
    ,@('Server1 ... ServerN',       'Drilling into a specific server. Each tab has the same 17-section structure.',                            '5-10 min each')
)
foreach ($row in $tabRows) {
    $how.Cells[$hr, 1].Value = $row[0]
    $how.Cells[$hr, 1].Style.Font.Bold = $true
    $how.Cells[$hr, 2].Value = $row[1]
    $how.Cells[$hr, 2].Style.WrapText = $true
    $how.Cells[$hr, 3].Value = $row[2]
    $how.Row($hr).Height = 28
    $hr++
}
$hr++

# ---- Section: Migration targets ----
$how.Cells[$hr, 1].Value = 'Migration targets explained'
$how.Cells[$hr, 1, $hr, 4].Merge = $true
$how.Cells[$hr, 1].Style.Font.Bold = $true
$how.Cells[$hr, 1].Style.Font.Color.SetColor([System.Drawing.ColorTranslator]::FromHtml('#FFFFFF'))
$how.Cells[$hr, 1].Style.Fill.PatternType = [OfficeOpenXml.Style.ExcelFillStyle]::Solid
...
# Header row for targets table
$how.Cells[$hr, 1].Value = 'Target'
$how.Cells[$hr, 2].Value = 'What it is'
$how.Cells[$hr, 3].Value = 'Best for'
$how.Cells[$hr, 4].Value = 'Caveats'
for ($c = 1; $c -le 4; $c++) {
    [int]$cInt = $c
    $hcell = $how.Cells.Item($hr, $cInt)
    $hcell.Style.Font.Bold = $true
...
$targetRows = @(
    ,@('DC-to-DC',
       'Stay on-prem, refresh hardware on the existing 5-year cycle.',
       'Workloads with hard data residency requirements or where cloud is not yet appropriate.',
       'Cost is rarely competitive vs cloud once you factor in DBA labor and refresh.')
    ,@('Azure VM (PAYG)',
       'Lift-and-shift to Azure VM with SQL license rolled into hourly rate.',
       'No existing license, short-term workloads, simplicity over cost.',
       'Most expensive Azure option for paid editions (Standard/Enterprise).')
    ,@('Azure VM (BYOL)',
       'Lift-and-shift to Azure VM with Azure Hybrid Benefit (existing license).',
       'Customers with active Software Assurance. Standard enterprise pattern.',
       'Requires verified existing license. For Developer Edition, same cost as PAYG.')
    ,@('Azure SQL MI',
       'Microsoft-managed SQL Server PaaS. No OS to patch.',
       'Workloads that fit MI feature set, where DBA labor savings justify higher unit price.',
       'Some features blocked: SSIS catalog, FileStream, UNSAFE CLR, certain Agent subsystems.')
)
foreach ($row in $targetRows) {
    ...
    $how.Cells[$hr, 2].Value = $row[1]
    $how.Cells[$hr, 2].Style.WrapText = $true
    $how.Cells[$hr, 2].Style.VerticalAlignment = [OfficeOpenXml.Style.ExcelVerticalAlignment]::Top
    $how.Cells[$hr, 3].Value = $row[2]
    $how.Cells[$hr, 3].Style.WrapText = $true
    $how.Cells[$hr, 3].Style.VerticalAlignment = [OfficeOpenXml.Style.ExcelVerticalAlignment]::Top
    $how.Cells[$hr, 4].Value = $row[3]
    $how.Cells[$hr, 4].Style.WrapText = $true
    $how.Cells[$hr, 4].Style.VerticalAlignment = [OfficeOpenXml.Style.ExcelVerticalAlignment]::Top
    $how.Row($hr).Height = 48
    $hr++
}
$hr++

# ---- Section: Reading a per-server tab ----
$how.Cells[$hr, 1].Value = 'Reading a per-server tab'
$how.Cells[$hr, 1, $hr, 4].Merge = $true
$how.Cells[$hr, 1].Style.Font.Bold = $true
$how.Cells[$hr, 1].Style.Font.Color.SetColor([System.Drawing.ColorTranslator]::FromHtml('#FFFFFF'))
$how.Cells[$hr, 1].Style.Fill.PatternType = [OfficeOpenXml.Style.ExcelFillStyle]::Solid
...
$perServer = @(
    'Top of tab: green-bordered Migration Recommendation block. Read the recommended target, the AI narrative, and the four cost cards (highlighted cell = recommended).',
    'Below that: 17 numbered sections of assessment data. Sections marked "(none)" mean the agent checked and found nothing - this is intentional, not a missing check.',
    'Rule of thumb: Section 4 (SQL Agent Jobs), Section 9 (CLR Assemblies), and Section 11 (Availability Groups) drive the most MI Verdict outcomes.',
    'Section 15 (Cloud Migration Matrix) at the bottom is the rule-based per-platform compatibility view (Yes/No with reason). It does NOT factor in cost - that is what the AI narrative at the top adds on.'
)
foreach ($line in $perServer) {
    $how.Cells[$hr, 1].Value = "* $line"
    $how.Cells[$hr, 1, $hr, 4].Merge = $true
    $how.Cells[$hr, 1].Style.WrapText = $true
    $how.Cells[$hr, 1].Style.VerticalAlignment = [OfficeOpenXml.Style.ExcelVerticalAlignment]::Top
...
$how.Cells[$hr, 1].Value = 'Color coding'
$how.Cells[$hr, 1, $hr, 4].Merge = $true
$how.Cells[$hr, 1].Style.Font.Bold = $true
$how.Cells[$hr, 1].Style.Font.Color.SetColor([System.Drawing.ColorTranslator]::FromHtml('#FFFFFF'))
$how.Cells[$hr, 1].Style.Fill.PatternType = [OfficeOpenXml.Style.ExcelFillStyle]::Solid
...
$colors = @(
    ...
    ,@('Amber',       'Conditional / Medium severity finding')
    ,@('Red',         'Blocked / High severity / risky configuration')
    ,@('Light blue',  'Section header rows')
    ,@('Navy blue',   'Banner / title rows')
)
foreach ($row in $colors) {
    $how.Cells[$hr, 1].Value = $row[0]
    $how.Cells[$hr, 1].Style.Font.Bold = $true
    $how.Cells[$hr, 2].Value = $row[1]
    $how.Cells[$hr, 2, $hr, 4].Merge = $true
    $how.Row($hr).Height = 18
    $hr++
}
$hr++

# ---- Section: When in doubt ----
$how.Cells[$hr, 1].Value = 'When in doubt'
$how.Cells[$hr, 1, $hr, 4].Merge = $true
$how.Cells[$hr, 1].Style.Font.Bold = $true
$how.Cells[$hr, 1].Style.Font.Color.SetColor([System.Drawing.ColorTranslator]::FromHtml('#FFFFFF'))
$how.Cells[$hr, 1].Style.Fill.PatternType = [OfficeOpenXml.Style.ExcelFillStyle]::Solid
...
$doubts = @(
    'A cost number looks wrong: open the Methodology & Sources tab. The rate that produced it is documented there.',
    'The AI recommendation looks wrong: open the per-server tab and read the narrative. The AI cites specific findings; you can verify against the 17 sections below.',
    'A server is missing: it failed to assess. Check PowerShell output for the connection error. Common causes: SPN/Kerberos issues, SQL service down, network path blocked.',
    'You disagree with a recommendation: that is fine. The AI is reasoning support, not the final word. Document your override in the migration plan that comes after this report.'
)
foreach ($line in $doubts) {
    $how.Cells[$hr, 1].Value = "* $line"
    $how.Cells[$hr, 1, $hr, 4].Merge = $true
    $how.Cells[$hr, 1].Style.WrapText = $true
    $how.Cells[$hr, 1].Style.VerticalAlignment = [OfficeOpenXml.Style.ExcelVerticalAlignment]::Top
...
# Column widths
$how.Column(1).Width = 22
$how.Column(2).Width = 36
$how.Column(3).Width = 30
$how.Column(4).Width = 30
...
# How to Read This Report goes BETWEEN Executive Summary and Summary - it's
# orientation that should be readable right after the high-level dashboard.
...
$pkg.Workbook.Worksheets.MoveToStart('How to Read This Report')
