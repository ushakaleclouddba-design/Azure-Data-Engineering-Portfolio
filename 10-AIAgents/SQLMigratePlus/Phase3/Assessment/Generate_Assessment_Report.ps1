...
# =============================================================================
# Parameters — added for SQLPilot Phase 2 .rpt upload path.
# =============================================================================
# Default mode (no parameters): the script runs against the SQL Server named
# in $SqlInstance / $CmsGroup below, exactly as before. The flow is
# unchanged.
#
# -InputRpt <path>   : Skips Invoke-Sqlcmd entirely. Parses the SSMS Results-
#                      to-Text (.rpt) file at <path> into the same in-memory
#                      $allData shape the live SQL path produces, then
#                      continues the existing pipeline (cost calc, AI sections,
#                      Cloud Migration Matrix, Excel build, JSON export).
#                      Output filename gets a '_FromUpload_' marker. The
#                      Methodology tab and JSON metadata record the source.
#
# -OutputDir <dir>   : Optional. Folder to write the .xlsx and .json into.
#                      Defaults to the script's own folder (existing behavior).
#                      Used by the SQLPilot UI to direct outputs into the
#                      assessment dir without changing CWD.
[CmdletBinding()]
param(
    # Accepts one or more .rpt paths. When more than one is supplied, the
    # parser runs on each in order and the resulting $allData hashtables are
    # merged into a single combined assessment — used to stitch a CMS-group
    # .rpt together with a separate single-server .rpt for the CMS host
    # (CMS hosts cannot register themselves in their own group).
    #
    # PowerShell array-binding accepts either:
    #   -InputRpt '.\estate.rpt'                       (single)
    #   -InputRpt '.\estate.rpt','.\host.rpt'          (multi)
    #   -InputRpt @('.\estate.rpt','.\host.rpt')       (explicit array)
    [string[]] $InputRpt  = @(),

    # Override the in-script $SqlInstance / $CmsGroup config. Used by the
    # SQLPilot UI to run the wrapper against any server without rewriting
    # the wrapper file. When passed, these values take precedence over the
    # values set below in the config section.
    [string]   $SqlInstanceOverride = '',
    [string]   $CmsGroupOverride    = '',

    [string]   $OutputDir = ''
)

# When the script is run by the SQLPilot UI (non-interactive background job),
# every Read-Host 'Press Enter to exit/close' would block forever waiting
# for keyboard input that never comes. This helper turns those prompts into
# no-ops in the non-interactive .rpt path while preserving the friendly
# prompt for human users running the script directly.
...
# If the caller passed -SqlInstanceOverride / -CmsGroupOverride (typically
# the SQLPilot UI's /api/assess/run endpoint), let them take precedence over
# the in-script defaults above. The non-empty check means an empty-string
# override doesn't accidentally clear a configured value.
if ($SqlInstanceOverride) { $SqlInstance = $SqlInstanceOverride }
if ($CmsGroupOverride)    { $CmsGroup    = $CmsGroupOverride }


# Output Excel filename. Defaults to a date-stamped name. When -InputRpt is
# passed, prefix with 'FromUpload_' so the artifact is instantly
# distinguishable from live-SQL runs in any folder listing.
if ($InputRpt) {
    $OutputXlsx = "Migration_Assessment_Report_FromUpload_$(Get-Date -Format 'yyyy-MM-dd_HHmm').xlsx"
} else {
    $OutputXlsx = "Migration_Assessment_Report_$(Get-Date -Format 'yyyy-MM-dd_HHmm').xlsx"
}

# Output directory:
#   - If the caller passed -OutputDir, honor that path exactly (used by the
#     SQLPilot UI when it wants to redirect anywhere).
#   - Otherwise default to '.\reports' so every run's xlsx + json lands in a
#     subfolder, keeping the assessment dir tidy.
#
# We DON'T create the directory yet — CWD at this point may be the caller's
# folder rather than the script's folder. The Set-Location below ensures CWD
# is the script's folder before we materialize the directory.
if (-not $OutputDir) {
    $OutputDir = '.\reports'
}
$OutputXlsx = Join-Path $OutputDir $OutputXlsx
...
# CWD is now the script's folder. Safe to materialize the relative
# $OutputDir ('.\reports' by default) and let the rest of the script
# write its xlsx + json into it.
if (-not (Test-Path -LiteralPath $OutputDir)) {
    New-Item -ItemType Directory -Path $OutputDir -Force | Out-Null
}

# Clear-Host throws "The handle is invalid" when this script is fired by a
# background launcher with no attached console (e.g. SQLPilot's
# Start-Process -WindowStyle Hidden). Silently swallow — the screen-clear is
# cosmetic for interactive use only.
...
if (-not $InputRpt -and $SqlInstance -eq 'CHANGE_ME') {
    ...
    Write-Host ''
    Wait-IfInteractive 'Press Enter to exit'
    exit 1
}

if (-not $InputRpt -and -not (Test-Path '01_Assessment_Script.sql')) {
...
if ($InputRpt) {
    Write-Host "Mode:         Parsing uploaded .rpt (Invoke-Sqlcmd skipped)" -ForegroundColor Cyan
    Write-Host "Source RPT:   $InputRpt"
} else {
...
# -----------------------------------------------------------------------------
# .rpt input mode — skip SQL discovery + SQL execution
# -----------------------------------------------------------------------------
# If -InputRpt was passed, the input is an SSMS Results-to-Text export of
# 01_Assessment_Script.sql. We parse it into the same $allData hashtable
# shape Invoke-Sqlcmd produces and skip everything SQL-related. The rest of
# the script (Excel build, AI sections, cost calc, JSON export) runs
# unchanged because it consumes $allData directly.
$allData = @{}
$serversToAssess = @()

if ($InputRpt -and $InputRpt.Count -gt 0) {
    # Dot-source the parser once. It lives next to this script.
    $parserPath = Join-Path $scriptDir 'rpt-to-datatables.ps1'
    if (-not (Test-Path -LiteralPath $parserPath)) {
        Write-Host "[ERROR] rpt-to-datatables.ps1 is missing from $scriptDir" -ForegroundColor Red
        Write-Host '        Drop the parser into the same folder as this script and try again.'
        Wait-IfInteractive 'Press Enter to exit'
        exit 1
    }
    . $parserPath

    # Resolve every input path up-front so any missing-file error surfaces
    # before we start parsing.
    $resolvedRpts = @()
    foreach ($rpt in $InputRpt) {
        if (-not (Test-Path -LiteralPath $rpt)) {
            Write-Host "[ERROR] -InputRpt file not found: $rpt" -ForegroundColor Red
            Wait-IfInteractive 'Press Enter to exit'
            exit 1
        }
        $resolvedRpts += (Resolve-Path -LiteralPath $rpt).ProviderPath
    }
    # Replace the parameter with the resolved list — downstream code
    # (filename marker, methodology rows, JSON metadata) re-uses $InputRpt.
    $InputRpt = $resolvedRpts

    Write-Host ''
    Write-Host ("Parsing {0} .rpt file(s)..." -f $InputRpt.Count) -ForegroundColor Cyan
    foreach ($rpt in $InputRpt) {
        Write-Host "  $rpt"
        try {
            $oneFile = Convert-RptToAllData -RptPath $rpt
        } catch {
            Write-Host "[ERROR] Failed to parse '$rpt': $($_.Exception.Message)" -ForegroundColor Red
            Wait-IfInteractive 'Press Enter to exit'
            exit 1
        }
        # Merge into $allData. If a server appears in more than one .rpt
        # (rare but possible if inputs overlap), the LATER file's data wins —
        # same last-write-wins behavior as a successive live-SQL re-assessment.
        foreach ($sname in $oneFile.Keys) {
            $allData[$sname] = $oneFile[$sname]
        }
        Write-Host ("    -> {0} servers" -f $oneFile.Count) -ForegroundColor Green
    }
    Write-Host ("  Total servers merged: {0}" -f $allData.Count) -ForegroundColor Green
    Write-Host ''

    # Empty $serversToAssess so the existing SQL loop iterates zero times.
    $serversToAssess = @()
}


# -----------------------------------------------------------------------------
# Resolve the list of servers to assess (live SQL path)
# -----------------------------------------------------------------------------
# Two modes:
#
...
# Build the live-SQL server list. Skip entirely when -InputRpt is set —
# in .rpt mode, $serversToAssess was already set to @() up above (so the
# SQL execution loop is a no-op) and $allData was populated by the parser.
if (-not $InputRpt) {
    $serversToAssess = @()
    if ($CmsGroup) {
...
function Convert-AllDataToJsonStructure {
    ...
    if ($SourceRpt -and $SourceRpt.Count -gt 0) {
        $metadata['run_mode'] = 'parsed_from_rpt'
        $leafNames = @($SourceRpt | ForEach-Object { Split-Path $_ -Leaf })
        if ($leafNames.Count -eq 1) {
            # Backward-compatible single-string form for single-file runs.
            $metadata['source_rpt'] = $leafNames[0]
        } else {
            # Multi-file: emit an array so consumers can render the file list.
            $metadata['source_rpts'] = $leafNames
        }
    }
...
# (Only initialize for the live-SQL path; the .rpt path already populated it.)
if (-not $InputRpt) {
    $allData = @{}
}
...
# When running from an uploaded .rpt (SQLPilot Phase 2 path), append provenance.
if ($InputRpt -and $InputRpt.Count -gt 0) {
    $runMeta += ,@('Run Mode',     'Parsed from uploaded .rpt (Invoke-Sqlcmd skipped)')
    if ($InputRpt.Count -eq 1) {
        $runMeta += ,@('Source RPT', (Split-Path $InputRpt[0] -Leaf))
    } else {
        $leafNames = @($InputRpt | ForEach-Object { Split-Path $_ -Leaf })
        $runMeta += ,@(('Source RPTs ({0})' -f $InputRpt.Count), ($leafNames -join '; '))
    }
}
...
$jsonOk     = $false
try {
    $jsonStructure = Convert-AllDataToJsonStructure -AllData $allData -SourceXlsxPath $OutputXlsx -SourceRpt $InputRpt
...
Write-Host ''
Wait-IfInteractive 'Press Enter to close'