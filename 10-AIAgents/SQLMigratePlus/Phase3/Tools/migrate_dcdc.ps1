...
function Invoke-DcDcStep {
    ...
    try {
        ...
        # Any non-ok step makes the overall run partial. Errors are caught below;
        # 'partial' and 'unavailable' must also bubble up so the top-level status
        # is honest about what happened inside.
        if ($st -eq 'error' -or $st -eq 'partial') { $Result.status = 'partial' }
    } catch {
