...
# real backend.
# ---------------------------------------------------------------------------
foreach ($name in 'kb.ps1','Phase1.ps1','restore.ps1','migrate_logins.ps1','migrate_tde.ps1','migrate_databases.ps1','migrate_jobs.ps1','migrate_linkedservers.ps1','migrate_configs.ps1','migrate_ssis.ps1','migrate_validate.ps1','migrate_dcdc.ps1','terraform.ps1','day2.ps1','decide.ps1','handoff.ps1') {
...
function Invoke-Route {
    ...
    # ---- Live database list for a source server (NOT from the stale assessment) ----
    # Migration must act on what exists RIGHT NOW, so query sys.databases live
    # rather than replaying a point-in-time assessment report. Cheap (ms).
    if ($method -eq 'GET' -and $path -eq '/api/source/databases') {
        $server = $Request.QueryString['server']
        if (-not $server) {
            Send-Json -Response $Response -StatusCode 400 -Body @{ error = "Missing 'server' query parameter (e.g. /api/source/databases?server=NODE1)" }
            return
        }
        try {
            $q = @"
SELECT d.name AS name, d.state_desc AS state, d.recovery_model_desc AS recovery,
       CASE WHEN d.is_encrypted = 1 THEN 1 ELSE 0 END AS is_encrypted,
       CASE WHEN EXISTS (SELECT 1 FROM sys.dm_hadr_database_replica_states rs WHERE rs.database_id = d.database_id) THEN 1 ELSE 0 END AS in_ag
FROM sys.databases d
WHERE d.database_id > 4
ORDER BY d.name;
"@
            $rows = Invoke-Sqlcmd -ServerInstance $server -Query $q -TrustServerCertificate -Encrypt Optional -ConnectionTimeout 8 -ErrorAction Stop
            $dbs = @()
            foreach ($r in @($rows)) {
                $dbs += @{ name="$($r.name)"; state="$($r.state)"; recovery="$($r.recovery)"; is_encrypted=([int]$r.is_encrypted -eq 1); in_ag=([int]$r.in_ag -eq 1) }
            }
            Send-Json -Response $Response -Body @{ status='ok'; server=$server; count=$dbs.Count; databases=$dbs; queried_at=(Get-Date).ToString('o') }
        } catch {
            Send-Json -Response $Response -StatusCode 500 -Body @{ status='error'; server=$server; error=$_.Exception.Message; note='Could not query the source live. Check the server name is reachable and you have access.' }
        }
        return
    }
    ...
    # ---- DC->DC migrate: launch as a background job ----
    # POST /api/migrate/dcdc
    # Body: { mode:'dcdc', scope:'whole'|'db', source, dest, databases:[...], engine:'dbatools'|'native' }
    # Mirrors /api/assess/run: Export-DbaInstance is slow (~8 min) and the
    # single-threaded HttpListener drops on long inline runs, so we shell out
    # to run_migrate_dcdc.ps1 as a detached process and poll for the result.
    if ($method -eq 'POST' -and $path -eq '/api/migrate/dcdc') {
        $argsBody = Read-JsonBody -Request $Request
        if (-not $argsBody -or -not $argsBody.source -or -not $argsBody.dest) {
            Send-Json -Response $Response -StatusCode 400 -Body @{ status='error'; error="Missing 'source' or 'dest'" }
            return
        }
        $source = "$($argsBody.source)".Trim()
        $dest   = "$($argsBody.dest)".Trim()
        $scope  = if ($argsBody.scope) { "$($argsBody.scope)".Trim() } else { 'whole' }
        if ($scope -ne 'whole' -and $scope -ne 'db') { $scope = 'whole' }
        $engine = if ($argsBody.engine) { "$($argsBody.engine)".Trim() } else { 'dbatools' }
        if ($engine -ne 'dbatools' -and $engine -ne 'native') { $engine = 'dbatools' }

        # Databases: only meaningful for scope='db'. Read-JsonBody gives an array.
        $databases = @()
        if ($argsBody.ContainsKey('databases') -and $argsBody.databases) {
            $databases = @($argsBody.databases | ForEach-Object { "$_".Trim() } | Where-Object { $_ })
        }
        if ($scope -eq 'db' -and $databases.Count -eq 0) {
            Send-Json -Response $Response -StatusCode 400 -Body @{ status='error'; error="scope='db' requires a non-empty 'databases' list" }
            return
        }

        # The runner that performs the migration in a detached process.
        # Lives in Tools\ alongside the tools it dot-sources.
        $runner = Join-Path $ToolsDir 'run_migrate_dcdc.ps1'
        if (-not (Test-Path $runner)) {
            Send-Json -Response $Response -StatusCode 503 -Body @{ status='error'; error="Migration runner not found at: $runner" }
            return
        }

        $jobId      = [Guid]::NewGuid().ToString('N').Substring(0,8)
        # Migration artifacts go to a PERMANENT, organized location under the
        # project root (not %TEMP%, which Windows can purge and which buries the
        # audit trail under a user profile). Filenames are self-describing:
        # <timestamp>_<source>-to-<dest>_<jobId>.{json,log}. This gives ARB a
        # replayable, discoverable audit record of every migration run.
        $migLogDir = Join-Path $ProjectRoot 'Logs\migrations'
        if (-not (Test-Path $migLogDir)) {
            New-Item -ItemType Directory -Path $migLogDir -Force | Out-Null
        }
        $stamp      = (Get-Date).ToString('yyyyMMdd_HHmmss')
        $safeSrc    = ($source -replace '[\\\/:*?"<>|]', '_')
        $safeDst    = ($dest   -replace '[\\\/:*?"<>|]', '_')
        $baseName   = "${stamp}_${safeSrc}-to-${safeDst}_$jobId"
        $logFile    = Join-Path $migLogDir "$baseName.log"
        $resultFile = Join-Path $migLogDir "$baseName.json"

        # Build the runner argument list. Pass databases as repeated tokens so
        # PowerShell binds them to the [string[]] $Databases parameter.
        $argsList = @(
            '-NoProfile','-ExecutionPolicy','Bypass',
            '-File', $runner,
            '-ToolsDir',    $ToolsDir,
        ...
        if ($scope -eq 'db' -and $databases.Count -gt 0) {
            # BUG-008 fix: pwsh -File mode does NOT honor [string[]] binding
            # with repeated tokens — only the first token after -Databases gets
            # bound, silently dropping the rest. Encode the list as base64-JSON
            # and pass as a single token; runner decodes it back to an array.
            # Base64 sidesteps all shell-quoting concerns (no commas, quotes,
            # spaces, or special chars to escape).
            $dbJson = ($databases | ConvertTo-Json -Compress)
            # ConvertTo-Json on a single-element array returns a scalar string
            # ("foo" not ["foo"]); force-wrap so the runner always sees an array.
            if ($databases.Count -eq 1) { $dbJson = '[' + $dbJson + ']' }
            $dbB64  = [Convert]::ToBase64String([Text.Encoding]::UTF8.GetBytes($dbJson))
            $argsList += '-DatabasesB64'
            $argsList += $dbB64
        }

        # Quote each element, doubling embedded single quotes.
        $startArgs = ($argsList | ForEach-Object { "'" + ($_ -replace "'", "''") + "'" }) -join ','
        $startInfo = "Start-Process -FilePath pwsh -ArgumentList $startArgs -RedirectStandardOutput '$logFile' -RedirectStandardError '$logFile.err' -WindowStyle Hidden -PassThru"
        $proc = Invoke-Expression $startInfo

        if (-not $script:MigrateJobs) { $script:MigrateJobs = @{} }
        $script:MigrateJobs[$jobId] = @{
            job_id      = $jobId
            pid         = $proc.Id
            started_at  = (Get-Date).ToString('o')
            source      = $source
        ...
        Publish-UiEvent -Type 'migrate_dcdc_start' -Payload @{
            job_id=$jobId; source=$source; dest=$dest; scope=$scope; engine=$engine
        }
        ...
        Send-Json -Response $Response -Body @{
            ...
            scope    = $scope
            engine   = $engine
            note     = "DC->DC migration running. Poll /api/migrate/dcdc/status?job_id=$jobId for progress."
            estimate_seconds = 480
        }
        return
    }

    # ---- DC->DC migrate: status of a running job ----
    # GET /api/migrate/dcdc/status?job_id=...
    if ($method -eq 'GET' -and $path -eq '/api/migrate/dcdc/status') {
        $jobId = $Request.QueryString['job_id']
        if (-not $jobId -or -not $script:MigrateJobs -or -not $script:MigrateJobs.ContainsKey($jobId)) {
            Send-Json -Response $Response -StatusCode 404 -Body @{ error = "Unknown job_id" }
            return
        }
        $job = $script:MigrateJobs[$jobId]

        # Is the runner process still alive?
        $procStillRunning = $false
        try { if (Get-Process -Id $job.pid -ErrorAction SilentlyContinue) { $procStillRunning = $true } } catch {}

        # Has the result file appeared? That's the authoritative "done" signal.
        $result = $null
        $haveResult = $false
        if (Test-Path $job.result_file) {
            try {
                $raw = Get-Content $job.result_file -Raw -ErrorAction Stop
                if (-not [string]::IsNullOrWhiteSpace($raw)) {
                    $result = $raw | ConvertFrom-Json
                    $haveResult = $true
                }
            } catch {
                # File may be mid-write; treat as not-yet-ready this poll.
            }
        }

        if ($haveResult) {
            $job.status = if ($result.status -eq 'error') { 'failed' } else { 'done' }
        } elseif (-not $procStillRunning) {
        ...
        $logTail = @()
        if (Test-Path $job.log_file) { $logTail = Get-Content $job.log_file -Tail 12 -ErrorAction SilentlyContinue }
        $errTail = @()
        if (Test-Path "$($job.log_file).err") { $errTail = Get-Content "$($job.log_file).err" -Tail 4 -ErrorAction SilentlyContinue }

        $elapsed = [int]((Get-Date) - ([DateTime]::Parse($job.started_at))).TotalSeconds

        if ($job.status -eq 'done' -or $job.status -eq 'failed') {
            Publish-UiEvent -Type 'migrate_dcdc_done' -Payload @{ job_id=$jobId; status=$job.status }
        }
    ...
    if ($method -eq 'POST' -and $path -eq '/api/handoff') {
        ...
        # BUG-010 fix: DC->DC handoff. When the client sends mode='dcdc' (with a
        # destination + database list), route to the DC->DC generator which skips
        # Terraform/Azure entirely. Otherwise fall through to the cloud path.
        $isDcDc = $args -and ("$($args.mode)" -eq 'dcdc')

        if ($isDcDc) {
            if (-not $args.source -or -not $args.dest) {
                Send-Json -Response $Response -StatusCode 400 -Body @{ error = "DC->DC handoff needs source and dest" }
                return
            }
            $have = Get-Command -Name 'Invoke-RealGenerateHandoffDcDc' -ErrorAction SilentlyContinue
            if (-not $have) {
                Send-Json -Response $Response -StatusCode 503 -Body @{ error = "DC->DC handoff tool not loaded" }
                return
            }
            try {
                $dbs = @()
                if ($args.databases) { $dbs = @($args.databases) }
                elseif ($args.database) { $dbs = @($args.database) }
                $params = @{
                    Source      = $args.source
                ...
                if ($args.migrate_result)  { $params['MigrateResult']  = (ConvertTo-Hashtable $args.migrate_result) }
                if ($args.validate_result) { $params['ValidateResult'] = (ConvertTo-Hashtable $args.validate_result) }
                $r = Invoke-RealGenerateHandoffDcDc @params
                Send-Json -Response $Response -Body $r
            } catch {
                Send-Json -Response $Response -StatusCode 500 -Body @{ status = 'error'; error = $_.Exception.Message }
            }
        ...
        # ---- Cloud handoff (original path, unchanged) ----
