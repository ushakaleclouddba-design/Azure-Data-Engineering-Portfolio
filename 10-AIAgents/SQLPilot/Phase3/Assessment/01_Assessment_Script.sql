...
;WITH JobSteps AS (
    ...
    WHERE j.name NOT IN (N'syspolicy_purge_history', N'SSIS Server Maintenance Job')
      AND j.name NOT LIKE N'sysutility[_]%'
      AND j.name NOT LIKE N'mdw[_]purge[_]%'
...
FROM (
    ...
    WHERE s.subsystem IN ('CmdExec','PowerShell','SSIS')
      AND j.name NOT IN (N'syspolicy_purge_history', N'SSIS Server Maintenance Job')
      AND j.name NOT LIKE N'sysutility[_]%'
      AND j.name NOT LIKE N'mdw[_]purge[_]%'
    GROUP BY j.name
) X;
...
DECLARE db_cur CURSOR LOCAL FAST_FORWARD FOR
    ...
    WHERE d.database_id > 4
      ...
      -- AG-role-proof guard: only loop databases this connection can actually
      -- USE right now. HAS_DBACCESS + DATABASEPROPERTYEX are evaluated live and
      -- are reliable mid-failover, unlike is_primary_replica (which can show
      -- stale/negative state during a role change and let a USE sever the batch).
